...
--  current pointer are stored in the Task Control Block in tasking runtimes
--  or in ``System.Secondary_Stack.Single_Task`` in non-tasking runtimes for
--  later retrieval. The secondary stack pointer is handled by increasing or
...
--  secondary stacks are allocated to the different tasks in tasking runtimes.
--  For non-tasking runtimes the secondary stack is lazily initialized on its
--  first use.
--
