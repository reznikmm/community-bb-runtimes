...
package body System.Generic_C_Math_Interface is
   ...
   --  will return a NaN value or raise an exception. Generally, for
   --  functions that require an exception to be raised for some
   --  arguments will also raise the same exception for NaN arguments.
   --  If there are multiple variants of a function with different numbers of
   --  arguments the behavior of NaN is identical for the same argument across
   --  different variants of the same function. A NaN argument that causes one
   --  variant of the function to return NaN will do so for all its variants
   --  even if this argument can raise an exception for some values.
   ...
   begin
      ...
      --  correct exception to be raised.

      --  Check Right explicitly for NaN since the if statement below cannot
      --  cover this case completely.
      if Right /= Right then
         raise Constraint_Error;
      end if;

      if not (Left > 0.0) then
         if not (Left = 0.0) or else Right = 0.0 then
   ...
   --    value zero or if one of the arguments is NaN
   ...
   is
   begin
      if (X = 0.0 and then Y = 0.0)
         or else X /= X or else Y /= Y
      then
         raise Argument_Error;
      else
   ...
   --  Exceptions:
   --    Argument_Error is raised when both X and Y have the value zero or
   --    when X or Y is NaN.
   ...
   --    or when Cycle <= 0.0 or Cycle, X or Y is a NaN.
   ...
   is
   begin
      if not (Cycle > 0.0) or else X /= X or else Y /= Y then
         raise Argument_Error;
      end if;
