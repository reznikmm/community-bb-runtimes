...
package body System.Secondary_Stack is
   ...
   begin
      ...
      --  We always align the top of the stack to either the requested
      --  alignment or Standard'Maximum_Alignment. Since we do not align the
      --  size of an allocation Top may not be aligned at the start of this
      --  procedure. This does not cause a performance impact since it saves
      --  the alignment check for both Top and the allocation. It also ensures
      --  the smallest possible memory overhead.

      --  Check if adding Storage_Size to Aligned_Top would overflow
      --  Aligned_Top. Not checking for an overflow here might cause
      --  Aligned_Top to wrap when this code is executed without runtime checks
      --  which would cause the following checks to miss the overflow.

      if SSE.Storage_Count (SS_Ptr'Last - Aligned_Top) < Storage_Size then
         raise Storage_Error;
      end if;

      --  Check if max stack usage is increasing

      if Stack.Max < Aligned_Top + SS_Ptr (Storage_Size) then
         ...
         if Stack.Size <= Aligned_Top + SS_Ptr (Storage_Size) - 1 then
            raise Storage_Error;
         end if;

         --  Record new max usage

         Stack.Max := Aligned_Top + SS_Ptr (Storage_Size);
      end if;

      --  Set resulting address and update top of stack pointer
      pragma Assert (Aligned_Top <= Stack.Internal_Chunk'Last);
      pragma Assert (Aligned_Top >= Stack.Internal_Chunk'First);
      ...
      Addr := Stack.Internal_Chunk (Aligned_Top)'Address;

      Stack.Top := Aligned_Top + SS_Ptr (Storage_Size);
   end SS_Allocate;
